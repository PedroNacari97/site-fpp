...
def nova_emissao(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for i in range(emissao.qtd_escalas):
                aid = request.POST.get(f"escala-{i}-aeroporto")
    ...
    aeroportos = list(Aeroporto.objects.values("id", "sigla", "nome"))
    return render(
        request,
...
def editar_emissao(request, emissao_id):
    ...
    escalas = []
    for e in emissao.escalas.all():
        total_sec = int(e.duracao.total_seconds())
        horas = total_sec // 3600
        minutos = (total_sec % 3600) // 60
        escalas.append(
            {
                "aeroporto": e.aeroporto_id,
    ...
    return render(
        ...
        "admin_custom/emissoes_form.html",
        {
            ...
            "aeroportos_json": json.dumps(list(Aeroporto.objects.values("id", "sigla", "nome"))),
        },
    )
