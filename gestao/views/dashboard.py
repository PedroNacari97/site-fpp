...
def _build_home_context(*, user, empresa=None, request=None):
    ...
    if request:
        start_date = timezone.datetime.strptime(request.GET.get("data_inicio"), "%Y-%m-%d").date() if request.GET.get("data_inicio") else start_date
        today = timezone.datetime.strptime(request.GET.get("data_fim"), "%Y-%m-%d").date() if request.GET.get("data_fim") else today
    selected_emissores = _selected_ids(request, "emissor") if request else []
    selected_clientes = _selected_ids(request, "cliente") if request else []
    selected_companhias = _selected_ids(request, "companhia") if request else []
    selected_programas = _selected_ids(request, "programa") if request else []
    selected_contas = _selected_ids(request, "conta") if request else []
