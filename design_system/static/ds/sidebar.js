...
window.NCflyShell = (function(){
  ...
  function resolveHref(href){
    // Permite sobrescrever hrefs via window.NC_DS_URLS (mapa {'admin-dashboard.html': '/design/admin-dashboard/'})
    if (typeof window.NC_DS_URLS === 'object' && window.NC_DS_URLS && window.NC_DS_URLS[href]) return window.NC_DS_URLS[href];
    return href;
  }
  function renderNav(navItems, active){
    return navItems.map(it => {
      ...
      return `<a class="nav-item ${isActive}" href="${resolveHref(it.href)}">
  ...
  function render(){
    ...
    const logoUrl = window.NC_DS_LOGO_URL || '../assets/ncfly-logo.svg';
    const brandHref = window.NC_DS_INDEX_URL || '#';
    root.outerHTML = `
      <div class="app">
        <aside class="sidebar">
          <div class="sidebar__brand"><a href="${brandHref}"><img src="${logoUrl}" alt="NCfly"></a></div>
